...
function hasMenuFeature(items: VfNavMenuItem[], predicate: (item: VfNavMenuItem) => boolean): boolean {
  return items.some((item) => predicate(item) || (item.children?.length ? hasMenuFeature(item.children, predicate) : false))
}
...
function handleSelect(item: VfNavMenuItem) {
  ...
  expandedValues.value = Array.from(new Set([...expandedValues.value, ...collectAncestorValues(props.items, item.value)]))
  emit('select', item)
}
...
<template>
  <nav :class="['vf-nav-menu', isSimpleMenu && 'vf-nav-menu--simple']" :aria-label="ariaLabel">
